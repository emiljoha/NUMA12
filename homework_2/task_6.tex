...
weight $w(x) = (1-x^2)^{-ˆ’1/2}$ on $[-1, 1]$, that is
\begin{equation*}
...
\begin{solution}  
Let us use the change of variables $x = \cos(\theta)$ and the definition of the Chebyshev polynomials
\begin{equation}
...
\begin{equation*}
-\frac{1}{2} \int_{\pi+2k\pi}^{2k\pi} (\cos((n-m)\theta) + \cos((n+m)\theta)) d\theta = -\frac{1}{2} \left [ \frac{1}{n-m} \sin ((n-m)\theta) + \frac{1}{n+m} \sin((n+m)\theta) \right ]_{\pi + 2k\pi}^{2k\pi}
\end{equation*}
Which is zero for al $k = 0,1,2,\ldots$
\end{solution}
